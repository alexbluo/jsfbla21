...
async function scrapePage(page, facetAmenities) {
    ...
    let pageData = {
        attraction_id: await tryQuerySelector(
            mainArticleSelector,
            "data-delid"
        ),
        attraction_name: await tryQuerySelector(
            mainArticleSelector,
            "data-dename"
        ),
        ...
        website_link: await tryQuerySelector(
            "a.button-gold.website-link",
            "href"
        ),
        directions_link: await tryQuerySelector(
    ...
    async function tryQuerySelectorAll(selector, tag) {
        const result = page.evaluate(
            (selector, tag) => {
                ...
                return tag
                    ? elements.map(
                          (element) => element[tag] || element.getAttribute(tag)
                      )
                    : elements;
            },
            selector,
